...
import type { RunSummary, TestResult, BangerConfig } from './types.js';
...
export class RunLogger {
  ...
  constructor(private readonly config: BangerConfig, private readonly cwd: string = process.cwd()) {
...
export function loadLatestRun(config: BangerConfig, cwd = process.cwd()): RunSummary | null {
  const runsBase = join(cwd, config.paths?.runs ?? 'runs');
...
export function loadRunById(runId: string, config: BangerConfig, cwd = process.cwd()): RunSummary | null {
